...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, mask=None, src_key_padding_mask=None,
                context=None, morphology_info=None):
        output = src
        for l in self.layers:
            output = l(output, src_mask=mask,
                       src_key_padding_mask=src_key_padding_mask,
                       context=context, morphology_info=morphology_info)
...
class TransformerEncoderLayerResidual(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048,
        ...
        if cfg.MODEL.TRANSFORMER.USE_MORPHOLOGY_INFO_IN_ATTENTION:
            self.connectivity_encoder = nn.Sequential(
                nn.Linear(3, 16), nn.ReLU(), nn.Linear(16, nhead))
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None,
                return_attention=False, context=None, morphology_info=None):
        src2 = self.norm1(src)

        if cfg.MODEL.TRANSFORMER.USE_MORPHOLOGY_INFO_IN_ATTENTION:
            src_mask = self.connectivity_encoder(morphology_info['connectivity'])
            src_mask = torch.cat([src_mask[:, :, :, i]
                                  for i in range(src_mask.size(-1))], 0)
