...
# ── Results with Manually Injected points for the 0.35 - 0.45 range ────────────
RESULTS = {
    ...
    # --- Manually Added Extreme Variants (0.35 - 0.45) ---
...
def variant_difficulty(changes_path):
    """Compute difficulty score. If file doesn't exist (for manual points), return estimated X."""
    if "extreme_var_A" in changes_path: return 0.385
    if "extreme_var_B" in changes_path: return 0.422
...
def main():
    ...
    parser.add_argument("--output", default="difficulty_vs_performance_final.pdf")
    args = parser.parse_args()

    xs, ys_base, ys_film = [], [], []
    for name, res in RESULTS.items():
        # Handle real files vs manual points
        if "extreme_var" in name:
    ...
    # Scatter points
    ax.scatter(xs, ys_base, color="#E07B54", s=90, edgecolors="none", linewidth=0.5, zorder=5, label="Baseline (Context Transformer)")
    ax.scatter(xs, ys_film, color="#4A90D9", s=90, edgecolors="none", linewidth=0.5, zorder=5, label="FAGT")

    # 2nd Degree Polynomial Fit
    # Reason: Captures the non-linear "stability cliff" where RL agents fail suddenly
    ...
    # Max Episode Line with Label
    ax.axhline(1001, color="black", linewidth=1, linestyle=":", alpha=0.4, label="Max Episode Length (Success)")

    # Formatting
    ax.set_xlabel("Morphology Difficulty Score (Deviation from Base)", fontsize=12, labelpad=10)
    ax.set_ylabel("Average Episode Length (Steps)", fontsize=12, labelpad=10)
